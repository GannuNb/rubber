...
import autoTable from 'jspdf-autotable'; // ✅ import as variableimport './AddLot.css';
import './AddLot.css';
...
const handleDownloadDetailedPDF = async () => {
  ...
  try {
    ...
    for (const lot of companyLots) {
      ...
      const finalRows = allRows.map((item, index) => {
        ...
        const remainingBalance =
          runningBalance >= 0
            ? `Amount Left: $ ${runningBalance.toLocaleString('en-US')}`
      ...
      doc.setFontSize(12);
      doc.text(`Lot - ${form.companyName} / ${lotNumber}`, 105, y, { align: 'center' });
      y += 6;
        ...
        didParseCell: function (data) {
          ...
          if (data.section === 'body') {
            if (type === 'Advance') {
              ...
              data.cell.styles.fillColor = [204, 255, 204]; // Green
            }
          }
        }
      });
    ...
    const remText =
      remaining < 0
        ? `Amount Left: $ ${Math.abs(remaining).toLocaleString('en-US')}`
